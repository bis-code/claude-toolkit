...
# Read worktree branch from .claude-toolkit.json if available
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --branch=*) WORKTREE_BRANCH="${1#*=}"; shift ;;
    -h|--help)
      echo "Usage: qa.sh [OPTIONS]"
      echo ""
      echo "Autonomous QA agent. Scans, fixes, and reports issues in a"
      echo "git worktree from the default branch."
      echo ""
      echo "Options:"
...
# State files live in the worktree's tools/qa/ directory
STATE_FILE="$WORKTREE_DIR/tools/qa/qa-state.json"
PROGRESS_FILE="$WORKTREE_DIR/tools/qa/qa-progress.txt"
MARKER_FILE="$WORKTREE_DIR/.qa-active"
...
for i in $(seq 1 "$MAX_LOOPS"); do
  ...
  # Spawn fresh Claude instance in the worktree directory
  OUTPUT=$(cd "$WORKTREE_DIR" && claude --dangerously-skip-permissions --print < "$WORKTREE_DIR/tools/qa/QA_PROMPT.md" 2>&1 | tee /dev/stderr) || true
